<template>
    ...
    <div class="modal fade" id="item-import-modal" data-backdrop="static">
...
export default{
  ...
  methods:{
    useToastr,
    async import_file(){
        let data = new FormData($('#itemForm')[0]);
        await this.axios.post(this.apiItem +'/import_items', data).then(res =>{
              this.useToastr().success('Items Added Successfully');
              this.$emit('importItemSuccess', true);
      }).catch((error)=>{
